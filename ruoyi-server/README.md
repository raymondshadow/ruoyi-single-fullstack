...
当前仓库已经补充了三套环境覆盖配置，位于 `ruoyi-admin/src/main/resources/`：

* `application-dev.yml`：本地开发
* `application-test.yml`：测试环境
* `application-prod.yml`：生产环境

主配置 `application.yml` 只保留公共配置，数据库、Redis、上传目录等环境差异全部放在对应环境文件中：

* 本地开发：`--spring.profiles.active=dev`
...
* Working directory：项目根目录 `ruoyi-server`
* VM options：`-Dfile.encoding=UTF-8`
* Program arguments：`--spring.profiles.active=dev`

说明：

* 日志目录已改为相对路径 `./logs`，避免 macOS / IDEA 下写死到 `/home/ruoyi/logs` 导致启动失败。
* 上传目录、MySQL、Redis 等本地参数优先改 `application-dev.yml`，不要直接改主配置。
